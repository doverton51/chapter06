...
module.exports = {
    ...
    setUsername: (userId, name) => redisClient.setAsync(`user:${userId}:name`, name),
    recordWin: userId => redisClient.zincrbyAsync('user:wins', 1, userId),
    getTopPlayers: () => redisClient.zrevrangeAsync('user:wins', 0, 2, 'withscores')
        .then(interleaved => {
            if(interleaved.length === 0) {
                return [];
            }

            let userIds = interleaved.filter((userId, index) => index % 2 === 0)
